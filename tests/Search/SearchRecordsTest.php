...
class SearchRecordsTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSearchOrder()
    {
        ...
        $this->assertEquals([1, 2, 3], array_keys($sort_by_num_desc));
        $this->assertEquals([3, 2, 1], array_keys($sort_by_num_asc));
        $this->assertEquals([2, 1, 3], array_keys($sort_by_text_desc));
        $this->assertEquals([3, 1, 2], array_keys($sort_by_text_asc));
        $this->assertEquals([3, 1, 2], array_keys($sort_by_date_desc));
        $this->assertEquals([2, 1, 3], array_keys($sort_by_date_asc));
    }

    public function testSearchIn()
    {
        $in_result = TestModelSortable::search()
            ->where('test_model_id', [1, 3], "IN")
    ...
    {
        $in_result = TestModelSortable::search()
            ->where('test_model_id', [1, 3], "NOT IN")
            ->order("test_model_id", "ASC")
            ->exec();
